...
- Input actions: wheel zoom, middle-drag pan, right-drag rotate,
  `Q`/`E` ±45° snaps, `Home` reset, WASD pan, double-click focus entity,
  `F` focus selection.
...
4. Wheel zoom keeps the world point under the cursor at the same screen
   position (solves the new target via plane intersection).
5. Camera position derives from `target` + fixed offset scaled by
   orthographic size — zooming never moves the focus point.
6. Follow mode preserves zoom and yaw (player-controlled only); any manual
   pan cancels follow.
7. No UI or gameplay code writes camera transforms; they *request* via the
   camera system's API.
...
pixels; follow a train then middle-drag — follow cancels. Automated:
headless test asserts pitch constant under simulated input and that zoom
clamps at band edges.
