...
- [x] 1.3 Write `art/config/world.toml` as the single source for `TILE_SIZE` and `HEIGHT_STEP` and verify it parses with the stdlib TOML reader
...
- [x] 3.2 Implement toolchain discovery for Godot and Blender via flag, `RR_GODOT`/`RR_BLENDER`, then known install locations, and verify an override is honoured and a missing toolchain prints the searched paths
...
- [x] 4.2 Wire `rr.py test` to the runner and verify a green suite exits 0 in under 30 seconds without opening a window
