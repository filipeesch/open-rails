...
**`RailService` owns validity, `RailNetwork` owns data.** A single `can_connect(cell, dir) -> String` is the only validity authority; it returns `""` when the connection is legal, otherwise a specific reason safe to show the player verbatim. The planner, the tool and future bridges all call it, so a reason never has to be reconstructed from a boolean. An error enum with a `message()` mapping was rejected: the reason *is* the player-facing string, so no layer gets a chance to restate it wrongly.
