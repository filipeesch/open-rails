...
**The speed scale is derived, not tuned.** `max_speed` is authored in km/h and reaches map units through one derived constant, `1 / (3.6 * TILE_METRES * TICK_RATE)`, whose metres-per-tile comes from `art/config/world.toml` — anchored on the shipped 4-4-0 being ~14 m over the buffers and built 0.85 tiles long, which puts a tile at 16 m. Acceleration and braking are the same physical quantities scaled the same way, and a train held inside its own braking distance creeps at a fraction of its top speed rather than an absolute figure. Alternative: a hand-picked tiles-per-tick rate — that was the code's original state, and it produced a valley where a "55 km/h" locomotive crossed the screen in a second and a one-step gradient stopped trains dead, because the limit constants had been tuned for a speed the conversion no longer produced.

**`ticks_per_day` is chosen against the rails, not against a taste for fast calendars.** A month must outlast a loaded leg: at twelve ticks a day a month came round every 18 seconds while a loaded leg measured eighty, so a pit pinned its platform ceiling and wasted everything above it before one consist could clear a single load — an economy that looks broken with every number in it correct. The shipped value is 60. The time-compression dial (1x/2x/4x) is the sanctioned answer to "faster", because it consumes more ticks per second without touching per-tick behaviour.
